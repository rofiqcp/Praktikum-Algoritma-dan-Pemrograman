...
from schemas.product import validate_product,validate_movement,exact_int,nonnegative_number
...
def normalize(payload,current=None):
    ...
    src={**(current or {}),**payload}
    data={"name":str(src["name"]).strip(),"category_id":exact_int(src["category_id"]),"price":nonnegative_number(src.get("price",0)),"stock":exact_int(src.get("stock",0))}
...
def move_stock(pid,payload):
    ...
    if e:raise ServiceError("VALIDATION_ERROR","Payload tidak valid",400,e)
    status,new_stock=repo.apply_stock_movement(pid,exact_int(payload["qty"]),str(payload.get("note","")))
